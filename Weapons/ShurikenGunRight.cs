...
public class ShurikenGunRight : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if ((rightController.controller.GetPressDown(rightController.triggerButton)/* && !rightShootingStarted*/))
        {
            rightShootingStarted = true;
            StartCoroutine("ShootRightGun");
        }
    }

    IEnumerator ShootRightGun()
    {
        InstantiateNewShuriken();
        AddForceAndTorqueToEquippedShuriken();
        rightController.PulseVibrate(10, 10, 0.01f, 1);
